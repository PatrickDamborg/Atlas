...
-- =============================================================================
-- Forge Atlas - Azure SQL Database
-- Applied: (record timestamp when executed)
-- Description: Creates all tables for the Forge Atlas application.
--
-- This migration is the canonical initial schema. For the full annotated
-- version with detailed comments, see: ../schema.sql
--
-- To apply:
--   sqlcmd -S <server>.database.windows.net -d <database> -U <user> -P <pass> -i 001_initial_schema.sql
-- =============================================================================

-- Guard: skip if already applied (check for projects table existence)
...
CREATE TABLE [dbo].[entity_fields] (
    ...
    [target_entities_json] NVARCHAR(MAX)    NULL,
...
CREATE TABLE [dbo].[entity_views] (
    [id]             UNIQUEIDENTIFIER NOT NULL DEFAULT NEWID(),
    [entity_id]      UNIQUEIDENTIFIER NOT NULL,
    [saved_query_id] NVARCHAR(100)    NOT NULL,
    [name]           NVARCHAR(255)    NOT NULL,
    [description]    NVARCHAR(MAX)    NULL,
...
CREATE TABLE [dbo].[app_modules] (
    ...
    [display_name]     NVARCHAR(255)    NOT NULL,
    [description]      NVARCHAR(MAX)    NULL,
    [app_id_dataverse] NVARCHAR(100)    NULL,
    [entity_count]     INT              NOT NULL DEFAULT 0,
    [entities_json]    NVARCHAR(MAX)    NULL,
    [sitemap_json]     NVARCHAR(MAX)    NULL,
...
CREATE TABLE [dbo].[app_tracks] (
    ...
    [entity_names_json]      NVARCHAR(MAX)    NULL,
...
CREATE TABLE [dbo].[proposed_workflows] (
    ...
    [related_entities_json]       NVARCHAR(MAX)    NULL,
...
CREATE TABLE [dbo].[training_content] (
    ...
    [completed_at]             DATETIME2        NULL,
    [brand_json]               NVARCHAR(MAX)    NULL,
    [screens_json]             NVARCHAR(MAX)    NOT NULL,
    [step_layouts_json]        NVARCHAR(MAX)    NOT NULL,
    [sitemap_structure_json]   NVARCHAR(MAX)    NOT NULL,
    [command_bar_actions_json] NVARCHAR(MAX)    NOT NULL,
    [solution_overview]        NVARCHAR(MAX)    NOT NULL,
    [tracks_json]              NVARCHAR(MAX)    NOT NULL,
    [documentation_pages_json] NVARCHAR(MAX)    NOT NULL,
...
-- ---------------------------------------------------------------------------
-- Seed Data: Default Consent Content
-- ---------------------------------------------------------------------------
INSERT INTO [dbo].[consent_content] (
    [id], [policy_version], [privacy_policy_version], [terms_version],
    [privacy_policy_title], [privacy_policy_content],
    [terms_title], [terms_content],
    [cookie_policy_title], [cookie_policy_content],
    [consent_prompt], [last_updated], [is_current]
) VALUES (
    NEWID(), '1.0.0', '1.0.0', '1.0.0',
    'Privacy Policy',
